...
use webservices\soap\types\SOAPBase64Binary;
use webservices\soap\types\SOAPHexBinary;
use webservices\soap\types\SOAPDateTime;
use webservices\soap\types\SOAPHashMap;
use webservices\soap\types\SOAPLong;
use webservices\soap\types\SOAPDouble;
...
class XPSoapNode extends Node {
  ...
  protected function _marshall($child, $value, $mapping) {
    ...
    if ($value instanceof \lang\types\String) {
      $value= (string)$value;
    ...
    if ($value instanceof \lang\types\Boolean) {
      $value= $value->value;
      // Fallthrough intended
    }

    if ($value instanceof \lang\types\Long) {
      $value= new SOAPLong($value->value);
      // Fallthrough
    }

    if ($value instanceof \lang\types\Integer) {
    ...
    if ($value instanceof \util\Date) {       // Date
      $value= new SOAPDateTime($value->getHandle());
      // Fallthrough intended
    }
    
    if ($value instanceof \util\Hashmap) {    // Hashmap
      $value= new SOAPHashMap($value->_hash);
      // Fallthrough intended
    }

    if ($value instanceof \webservices\soap\types\SoapType) {   // Special SoapTypes
    ...
    if (($value instanceof \lang\Generic) && null !== ($qname= $mapping->qnameFor($value->getClass()))) {
      $ns++;
