...
class SOAPHTTPTransport extends AbstractRpcTransport {
  ...
  public function send($message) {
    if (!$message instanceof \webservices\soap\xp\XPSoapMessage) {
      throw new \lang\IllegalArgumentException(__METHOD__.' expects webservices.soap.xp.XPSoapMessage, but got '.\xp::typeOf($message));
    }
    ...
    // Post XML
    with ($request= $this->_conn->create(new \peer\http\HttpRequest())); {
      $request->setMethod(HttpConstants::POST);
      $request->setParameters(new \peer\http\RequestData(
  ...
  public function retrieve($response) {
    ...
    switch ($code) {
      ...
      case HttpConstants::STATUS_INTERNAL_SERVER_ERROR:
        ...
        $answer= \webservices\soap\xp\XPSoapMessage::fromString($xml);
